...
class PolygonClient:
    ...
    def fetch_news(self, start_date: str, end_date: str, limit: int = 100) -> List[Dict[str, Any]]:
        ...
        while True:
            ...
            if not next_url:
                break
            # next_url already contains the apiKey, so drop it from params
            url_path = next_url
            params = None
...
class FinnhubClient:
    """Minimal client for the Finnhub API."""
...
def main():
    ...
    fh = FinnhubClient(api_key=cfg.finnhub_api_key)
    ...
    logging.info("Fetched %d SPY constituents", len(spy_constituents))
